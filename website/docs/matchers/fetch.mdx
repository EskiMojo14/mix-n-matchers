...
Use `.toHaveResponseType(expected)` to assert that a `Response` object has the expected type (e.g., `"basic"`, `"cors"`, `"error"`, `"opaque"`, `"opaqueredirect"`).
...
Use `.toHaveSearchParam(name, value?)` to assert that a `URLSearchParams`, `URL`, `Response`, or `Request` object has the specified query parameter. If `value` is provided, it will also check that the parameter value matches.
