...
Use `.toHaveFormDataBody(expected)` to assert that a `Response` or `Request` object has the expected `FormData` body, using [`formDataEquality`](/mix-n-matchers/utilities/equality#formDataEquality).

<TestFile name="toHaveFormDataBody">
{`
